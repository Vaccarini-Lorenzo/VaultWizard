...
export class AzureResponsesSseReader {
    ...
    async consume(response: Response, onChunk: (chunk: string) => void): Promise<void> {
        ...
        while (true) {
            ...
            for (const eventBlock of eventBlocks) {
                this.processEventBlock(eventBlock, onChunk);
            }
        }
    }

    private processEventBlock(eventBlock: string, onChunk: (chunk: string) => void): void {
        ...
        for (const line of lines) {
            ...
            try {
                ...
                const deltaText = this.textExtractor.extractDelta(eventJson);
                if (deltaText) {
                    onChunk(deltaText);
                }
            } catch {
